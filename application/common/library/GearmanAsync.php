...
class GearmanAsync
{
     ...
     * ]
     */
    public function resourceUpload($job = array())
    {
        $job = json_encode($job);
        //不等待返回结果，就会结束
        $this->client->doBackground("resourceUpload", $job);
    }
    ...
    public function sendMessage($messages)
    {
        ...
        foreach ($messages as $message) {
            ...
            foreach ($message['users'] as $user) {
                if (!empty($user['email']) && !in_array($user['email'], $all_email)) {
                    $email[] = $user['email'];
                    $all_email[] = $user['email'];
                }
                if (!empty($user['telephone']) && !in_array($user['telephone'], $all_mobile)) {
                    $mobile[] = $user['telephone'];
                    $all_mobile[] = $user['telephone'];
