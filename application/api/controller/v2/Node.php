...
class Node extends Api
{
    ...
    public function getDetailList()
    {
        ...
        if($list)
        {
            $this->returnmsg(200,'Operation succeeded!',$list);
        }

        $this->returnmsg(402,'Data illegal！');
    }
    ...
    public function gotoNext()
    {
        ...
        if($nodeId)
        {
            ...
            $doc_res = Document::docAuditInfo(['node_id'=>$nodeId,'status' => 1,'submit_status' => 1],'audit_status');
            $ud_res = UitableviewDefault::udAuditData(['node_id' => $nodeId,'status' => 1,'submit_status' => 1], 'audit_status');
            $bom_res = ProjectBom::BomAuditInfo(['node_id' => $nodeId,'status' => 1,'submit_status' => 1], 'audit_status');

            //所有文档是否已提交并已审核，转至下一流程，否则反之
            if($doc_res && $ud_res && $bom_res)
            {
                ...
                {
                    $this->returnmsg(401,'请检查参数是否符合要求','');
                }
                $this->returnmsg(200,'success！','');
            }
            else
            {
                $this->returnmsg(401,'所有提交的文档状态为已审核才能转入一个流程','');
            }
        }
        $this->returnmsg(401,'参数错误');
    }
        ...
        if($nodeId)
        {
            ...
            if(!empty($nodeArr))
            {
                ...
                if(!$nodeArr['status'])
                {
                    ...
                    //计算节点完成时间
                    Project::get($nodeArr['project_id'])->calculateProjectTime();
                    ...
                    if($map)
                    {
                        Db::commit();
                        //编辑节点成功，发送邮件、短信通知
                        ProjectNode::sendEditNodeMes($nodeId);
                        $this->returnmsg(200, 'success！' );
                    }
                    Db::rollback();
                    $this->returnmsg(402,'编辑失败','');
                }
                else
                {
                    $this->returnmsg(402,'节点状态为未开始才能开始编辑');
                }
            }
            else
            {
                $this->returnmsg('402','数据库无此记录');
            }
        }else{
            $this->returnmsg(401, '参数错误');
        }
