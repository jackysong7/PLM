    ...
    public static function getNodeDetail($projectId)
    {
        ...
        foreach($info as $k=>$v) {
            ...
            $bomInfo = Db::name('project_bom')
                ->field('creator_id,audit_status,bom_id')
                ->where(['node_id' => $v['node_id'], 'project_id' => $projectId])
                ->order('bom_id desc')
                ->select();
            if (!empty($bomInfo)) {
                foreach ($bomInfo as $bom) {
                    $map['target_id'] = $bom['bom_id'];
                    $map['target_type'] = 6;//文档类型
                    $map['role_type'] = 5;//审核人员
                    $auditorBom = Relation::auditInfo($map);
                    if ($auditorInfo != false && $auditorBom != false) {
                        foreach ($auditorInfo['auditor'] as &$item) {
                    ...
                    $bomUploader = Relation::getNickName($bom['creator_id']);
                    $data[$k]['doc_list'][] = [
                        'doc_name' => 'BOM文档',
                        'status' => $bom['audit_status'],
                        'uploader' => isset($bomUploader) ? ['admin_id' => $bom['creator_id'], 'nickname' => $bomUploader] : '',
            ...
            $udInfo = Db::name('uitableview_default')
                ->field('creator_id,audit_status,ud_id')
                ->where(['node_id' => $v['node_id'], 'project_id' => $projectId])
                ->order('ud_id desc')
                ->select();
            if (!empty($udInfo)) {
                foreach ($udInfo as $ud) {
                    // 立项表审核信息
                    $map['target_id'] = $ud['ud_id'];
                    $map['target_type'] = 6;//文档类型
                    $map['role_type'] = 5;//审核人员
                    $auditorUd = Relation::auditInfo($map);
                    if ($auditorInfo != false && $auditorUd != false) {
                        foreach ($auditorInfo['auditor'] as &$item) {
                    ...
                    $udUploader = Relation::getNickName($ud['creator_id']);
                    $data[$k]['doc_list'][] = [
                        'doc_name' => '开发立项表',
                        'status' => $ud['audit_status'],
                        'uploader' => isset($udUploader) ? ['admin_id' => $ud['creator_id'], 'nickname' => $udUploader] : '',
                        'auditor' => isset($auditorInfo['auditor']) ? $auditorInfo['auditor'] : ''
                    ];
                }
            }
            ...
            $docInfo = Db::name('document')
                ->field('file_name doc_name,admin_id,audit_status,pd_id')
                ->where(['node_id' => $v['node_id'], 'project_id' => $projectId])
                ->order('pd_id desc')
                ->select();
            if (!empty($docInfo)) {
                foreach ($docInfo as $doc) {
                    // 文档审核信息
                    $map['target_id'] = $doc['pd_id'];
                    $map['target_type'] = 3;//文档类型
                    $map['role_type'] = 5;//审核人员
                    $auditorDoc = Relation::auditInfo($map);
                    if ($auditorInfo != false && $auditorDoc != false) {
                        foreach ($auditorInfo['auditor'] as &$item) {
                    ...
                    $docUploader = Relation::getNickName($doc['admin_id']);
                    $data[$k]['doc_list'][] = [
                        'doc_name' => $doc['doc_name'],
                        'status' => $doc['audit_status'],
                        'uploader' => isset($docUploader) ? ['admin_id' => $doc['admin_id'], 'nickname' => $docUploader] : '',
                        'auditor' => isset($auditorInfo['auditor']) ? $auditorInfo['auditor'] : ''
                    ];
                }
