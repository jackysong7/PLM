...
//                ->where('submit_status',0)
        ...
        elseif ($nodeId && $submitStatus)//已提交
        {
            ...
            $bomList = Db::name('project_bom')
                ->field('bom_id doc_id,tpl_name doc_name,tpl_name,tpl_id,tpl_type,add_time,submit_time,audit_status,audit_time,version,creator_id,reedit')
//                ->where('node_id',$nodeId)
//                ->where('submit_status',1)
            ...
            $udList = Db::name('uitableview_default')
                ->alias('u')
                ->field('u.ud_id doc_id,u.tpl_id,t.tpl_name doc_name,t.tpl_type,u.create_at add_time,u.submit_time,u.audit_status,u.audit_time,u.version,u.creator_id,u.reedit')
                ->join('doc_template t','u.tpl_id=t.tpl_id','LEFT')
                ->where('u.node_id',$nodeId)
            ...
            $docList = Db::name('document')
                ->alias('d')
                ->field('d.pd_id doc_id,d.file_name doc_name,d.admin_id,d.tpl_id,t.tpl_name,t.tpl_type,d.createtime add_time,d.submit_time,d.audit_status,d.audit_time,d.version,d.reedit')
                ->join('doc_template t','d.tpl_id=t.tpl_id','LEFT')
//                ->where('d.node_id',$nodeId)
//                ->where('d.submit_status',1)
//                ->where('d.tpl_id','>',0)
//                ->where('d.status',1)
                ->where("d.node_id=$nodeId AND d.submit_status=1 AND d.tpl_id>0 AND d.status=1")
                ->order('pd_id desc')
                ->select();
            if($docList)
            {
    ...
    {
        return Db::name('project_bom')->where($where)->field($field)->order('bom_id desc')->select();
    }
    ...
    public static function saveErpBom($bomId)
    {
        ...
        foreach ($sBomMaterialList as $sBomMaterial) {
            ...
            $FTreeEntity['FPOSITIONNO'] = $sBomMaterial['location_num']; //位置号
