...
class Admin extends Model
{
    ...
    public static function getInfo($params){
        ...
        $result = self::initConnect()
            ->name('admin')
            ->field('admin_id,username,nickname,email,telephone,is_boss')
            ->where($where)
            ->find();
...
//                $rules = explode(',',$admin['job_rules']);
    ...
    public static function add($params){
        $data = [
            ...
            'is_boss'=>(isset($params['is_boss']) ? $params['is_boss'] : 0),
        ];
    ...
    public static function edit($params){
        ...
        if($admin){
            ...
            // 是否是boss
            if (isset($params['is_boss']) && in_array(intval($params['is_boss']), [0,1]))
            {
                $admin_info['is_boss'] = intval($params['is_boss']);
            }

            $result = self::initConnect()->name('admin')->where('admin_id',$params['admin_id'])->update($admin_info);
            if (empty(Db::name('admin_info')->where('admin_id',$params['admin_id'])->find()))
            {
                Db::name('admin_info')->insert([
