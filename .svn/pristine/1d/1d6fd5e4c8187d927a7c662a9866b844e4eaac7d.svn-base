...
-  DEMO: user控制器是继承了Api父类，会在初始化检测user控制器定义的访问限制 $restMethodList = 'get|post|put';
-  在Api类中，会有方法checkAuth()检测用户是否有权限调用接口
-  checkAuth方法会调用Oauth类中的鉴权，$baseAuth = Factory::getInstance(\app\api\controller\OAuth::class);
-  根据用户端传递过来的app_key生成或者获取缓存中的access_token，接口地址：/api/v1/token/access_token
-  提交参数app_key和access_token进行对比，如果true，则可以调用user中的各种方法，否则返回不能调用原因
