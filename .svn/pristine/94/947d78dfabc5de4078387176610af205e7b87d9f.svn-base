...
class ProjectMsg extends Api
{
    ...
    public function getList($data)
    {
        ...
        if (!empty($data['get_my'])) {
            $where['a.admin_id'] = Auth::instance()->getUser()['admin_id'];
        }
        if (!empty($data['process_id'])) {
            $where['a.process_id'] = $data['process_id'];
        }
        $where['a.status'] = 1;
        $where['b.status'] = 1;

        $projectMsg = new \app\api\model\ProjectMsg();
        $totalNumber = $projectMsg->alias('a')->join('plm_project b', 'a.project_id = b.project_id')->field('a.*')->where($where)->count();
        try {
            $list = $projectMsg->alias('a')->join('plm_project b', 'a.project_id = b.project_id')->field('a.*')->where($where)->page($data['page_no'], $data['page_size'])->select();
        } catch (DbException $e) {
